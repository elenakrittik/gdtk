...
pub(crate) fn convert<'a, T: std::str::FromStr>(lexer: &logos::Lexer<'a, TokenKind<'a>>) -> logos::Filter<T> {
...
pub(crate) fn convert_radix<'a, const R: u32>(lexer: &logos::Lexer<'a, TokenKind<'a>>) -> logos::Filter<u64> {
...
pub(crate) fn trim_quotes<'a, const SKIP_PREFIX: bool>(lexer: &logos::Lexer<'a, TokenKind<'a>>) -> &'a str {
    if SKIP_PREFIX {
        &lexer.slice()[1..]
    } else {
        lexer.slice()
