...
impl<'a> FuzzySelect<'a> {
    ...
    pub fn with_default(self, val: usize) -> Self {
        Self {
            default: Some(val),
            ..self
        }
    ...
    pub fn add_item<T: ToString>(mut self, item: T) -> Self {
        self.items.push(item.to_string());
        self
    }

    /// Adds multiple items to the fuzzy selector.
    pub fn add_items<T: ToString>(mut self, items: &[T]) -> Self {
        self.items.extend(items.iter().map(|v| v.to_string()));
        self
    }
